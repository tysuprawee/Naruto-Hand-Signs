...
> ⚠️ **Note — Files Not Included in This Repo**
>
> To keep the repository lightweight, the following are **excluded via `.gitignore`** and must be obtained separately:
>
> | Excluded | What It Is | How to Get It |
> |----------|-----------|---------------|
> | `models/*.task`, `*.tflite`, `*.pt` | Trained ML model weights (MediaPipe, YOLO) | Download from the [Releases](https://github.com/tysuprawee/Naruto-Hand-Signs/releases) page or train your own with `src/mp_trainer.py` |
> | `models/runs/` | YOLO training run outputs | Generated during training |
> | `dataset/` | Training images & labels (~3K+ files) | Collect with `src/capture_dataset.py` |
> | `web/public/*.wasm` | ONNX Runtime WASM binaries (~61 MB) | `npm install` in `web/` fetches these, or copy from `node_modules/onnxruntime-web/dist/` |
> | `web/public/*.csv` | MediaPipe KNN reference data (~50 MB) | Generated by the training pipeline |
> | `*.zip` | Portable build archives | Available on the [Releases](https://github.com/tysuprawee/Naruto-Hand-Signs/releases) page |
>
> **Want to just play?** Download the ready-to-run portable build from the [Releases](https://github.com/tysuprawee/Naruto-Hand-Signs/releases) page — no setup required.
...
│   │   ├── download/mac/page.tsx       # macOS download page
...
├── models/                             # ML model weights (⚠️ gitignored)
├── dataset/                            # Training data (⚠️ gitignored)
